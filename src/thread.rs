...
// static CURRENT: &[Mutex<Option<ThreadBox>>] = &[Mutex::new(None)];
// static CURRENT: Option<ThreadBox> = None;
...
pub fn _yield() {
    // let thread = Box::from_raw(current())
...
pub fn spawnf<F: 'static + FnMut()>(mut f: F, uart: &Mutex<Option<UART>>) {
    // Wait until there is a free CPU in the bit map
    let mut used_cpus = USED_CPUS.load(Ordering::Relaxed);
    let mut next_cpu;
    loop {
    ...
    uart.map(|u| {
        // writeln!(u, "{:x}", unsafe { *a as  })
        writeln!(u, "{:x}", conf as *const _ as u64)
    });
